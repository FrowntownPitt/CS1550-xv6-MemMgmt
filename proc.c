...
procdump(void)
{
    ...
    //acquire(&ptable.lock);
    cprintf("\n");
    cprintf("AMP = Allocated Memory Pages \n");
    cprintf("PO = Paged Out \n");
    cprintf("PF = Paged Faults \n");
    cprintf("\n");
    cprintf("\n pid \t| state \t| name \t\t| Size \t| AMP \t| PO \t| PF \t| #PO \t| \n");
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
        if(p->state == UNUSED)
        continue;
        ...
        // Want to get the current available pages in the processes physical memory
        x = p->nPhysPages;
        currAvailable += x;
        cprintf(" %d \t| %s \t| %s\t\t| %d \t| %d \t| %d \t| %d \t| %d \t|", p->pid, state, p->name,
                p->sz/PGSIZE, p->nPhysPages, p->nPages, p->pageFaults, p->pageSwapped);
        if(p->state == SLEEPING){
